...
def Mask_Generate(lengths: torch.Tensor, max_length: Union[torch.Tensor, int, None]=None):
    '''
    lengths: [Batch]
    max_length: an int value. If None, max_length == max(lengths)
    '''
    try:
        # 길이값 검사: 음수/NaN/0 항목 있는지 방어 (필요시)
        if torch.any(lengths <= 0) or torch.any(torch.isnan(lengths)):
            raise ValueError("Invalid 'lengths' in Mask_Generate!")

        max_length = max_length or int(torch.max(lengths).item())
        sequence = torch.arange(max_length)[None, :].to(lengths.device)
        mask = sequence >= lengths[:, None]    # [Batch, Time]
        return mask
    except Exception as e:
        print(f"[Mask_Generate] 마스크 생성 에러, 전체 마스킹(True) 반환: {e}")
        # 임의: 배치 전체를 마스킹 (True)
        batch_size = lengths.shape[0] if hasattr(lengths, "shape") else 1
        time_length = int(max_length) if max_length is not None else 1
        return torch.ones((batch_size, time_length), dtype=torch.bool, device=lengths.device)